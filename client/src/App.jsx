...
import { Send, Code, Eye, Loader2, History } from 'lucide-react';
...
const App = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Error generating UI. Check console for details.");
    }
    setLoading(false);
  };
  ...
  const fetchVersions = async () => {
    try {
     const res = await axios.get(`${API_URL}/api/versions`);
      ...
      console.error("No backend connection"); 
    }
  };
                ...
                <div className="space-y-2">
                   {versions.map((v) => (
                      <button 
                        ...
                        onClick={() => {
                            setCode(v.code); 
                            setExplanation(`Rolled back to: ${v.prompt}`);
                        }}
